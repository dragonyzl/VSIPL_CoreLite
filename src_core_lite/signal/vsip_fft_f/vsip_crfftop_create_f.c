                       ...
                       vsip_alg_hint hint)
{
    ...
    fft->temp = vsip_cvcreate_f(3 * fft->N + 3,0); /* create some work space */
    fft->wt = vsip_cvcreate_f(fft->N,0);
    ...
    { 
        ...
        vsip_vramp_f(0, VI_ft_f_2PI/fft->N,wtR);  
        vsip_vsin_f(wtR,wtI);
        vsip_vcos_f(wtR,wtR);
    }
    ...
    fft->temp->length = fft->N + 1;
    {
        ...
        vsip_vramp_f(0, VI_ft_f_PI/fft->N,wtR);
        vsip_vsin_f(wtR,wtI);
        vsip_vcos_f(wtR,wtR);
    }
    fft->temp->offset = 0;
    ...
    /* calculate unsorting vector */
    {
        ...
        for(i = 0; i< fft->N; i++) fft->index[i] = VI_jofk_f(i,fft->pn,fft->p0,pF,fft->length);
    }
    return fft;
}
