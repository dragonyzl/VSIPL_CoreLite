...
**********************************************************************/
/* ccfftop_create */
/* create fft object for use with ccfftop */
#include<vsip.h>
...
vsip_fft_f* vsip_ccfftop_create_f(vsip_length N,
                       vsip_scalar_f scale,
                       vsip_fft_dir dir,
                       unsigned int ntimes,
                       vsip_alg_hint hint)
{
    ...
    fft->temp = vsip_cvcreate_f(N,0); /* create a space to copy too when unsorting */
    fft->wt = vsip_cvcreate_f(N,0);
    ...
    { 
        ...
        if(dir == -1){ 
          vsip_vramp_f(0,- VI_ft_f_2PI/N,wtR);  
        }else{ 
          vsip_vramp_f(0, VI_ft_f_2PI/N,wtR); 
        }   
        vsip_vsin_f(wtR,wtI);
    ...
    /* calculate unsorting vector */
    {
        ...
        for(i = 0; i< N; i++) fft->index[i] = VI_jofk_f(i,fft->pn,fft->p0,pF,fft->length);
    }
    return fft;
}
/* nuV*/
...
vsip_length VI_nuV_f(vsip_length N, 
         vsip_scalar_vi *pn,
         vsip_scalar_vi *p0,
         vsip_scalar_vi *pF)
{
...
vsip_scalar_vi VI_jofk_f(vsip_scalar_vi k, /* index to reverse */
                    vsip_scalar_vi *pn,
                    vsip_scalar_vi *p0,
                    vsip_scalar_vi pF,
                    vsip_length fn)
