...
/* SPAWARSYSCEN D881 */
/* Top level fft routine for mixed factors of 2,3,4,5,7,8*/
/*========================================================*/
/* dft */
/******************************************************************/

void VI_dft_f(vsip_fft_f *fft) {
...
void VI_ifft2_f(vsip_fft_f* fft)
{
   vsip_scalar_f *xp_r = fft->xp_r,
                 *xp_i = fft->xp_i; 
   vsip_scalar_vi xpst = fft->xpst;
...
void VI_ifft3_f(vsip_fft_f *fft)
{
   vsip_scalar_f *xp_r = fft->xp_r,
                 *xp_i = fft->xp_i;
   vsip_scalar_vi xpst = fft->xpst;
...
void VI_ifft4_f(vsip_fft_f *fft)
{
   vsip_scalar_f *xp_r = fft->xp_r,
                 *xp_i = fft->xp_i;
   vsip_scalar_vi xpst = fft->xpst;
...
void VI_ifft5_f(vsip_fft_f *fft)
{
   vsip_scalar_f *xp_r = fft->xp_r,
                 *xp_i = fft->xp_i;
   vsip_scalar_vi xpst = fft->xpst;
...
void VI_ifft7_f(vsip_fft_f* fft)
{
   vsip_scalar_f *xp_r = fft->xp_r,
                 *xp_i = fft->xp_i;
   vsip_scalar_vi xpst = fft->xpst;
...
void VI_ifft8_f(vsip_fft_f* fft)
{
   vsip_scalar_f *xp_r = fft->xp_r,
                 *xp_i = fft->xp_i;
   vsip_scalar_vi xpst = fft->xpst;
...
void VI_fft_wt_f(vsip_fft_f *fft)
{
      vsip_cvview_f *a = fft->x;
      vsip_cvview_f *b = fft->wt;
...
void VI_p0pF_f(vsip_fft_f *fft)
{
  ...
  void (*fft_p)(vsip_fft_f*) = VI_p0pF_f;
...
/* init fft to x */
void init_fft(vsip_cvview_f *x,vsip_fft_f* fft){
  fft->x = x;
  fft->stage =0;
  return;
}

void VI_ccfftip_f(const vsip_fft_f* fft, const vsip_cvview_f *x)
{
  init_fft((void*)x,(void*)fft);
  if(fft->dft == 1){
     VI_dft_f((void*)fft);
  }else{
     VI_p0pF_f((void*)fft);
     VI_sort_copy_f((void*)fft);
  }
...
void VI_sort_copy_f(vsip_fft_f* fft){
...
/*========================================================*/
/* VSIP Implementer cvgather.  Required in Core Lite because */
/* there are no index vectors supported, so cvgather is not supported */
void (VI_cvgather_f)( 
  const vsip_cvview_f* a,
  const vsip_scalar_vi *xp,
