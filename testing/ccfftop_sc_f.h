...
**********************************************************************/
/* $Id: ccfftop_sc.t,v 1.2 2000/04/12 20:46:12 judd Exp $ */

static void ccfftop_sc_f()
{
   printf("*********\nTEST ccfftop forward, special case with strided input and output\n");
   {
      vsip_length MAX_POINTS=24;
      vsip_length np[]={8,16,22,24},npm,n2;
      int i;
      vsip_scalar_f enp,t,err;
      vsip_cblock_f *block = vsip_cblockcreate_f(MAX_POINTS*7,0);
      vsip_cvview_f *x1 = vsip_cvbind_f(block,MAX_POINTS*3,-3,MAX_POINTS);
      vsip_cvview_f *y1 = vsip_cvbind_f(block,MAX_POINTS*3+1,2,MAX_POINTS);
      vsip_cvview_f *x = vsip_cvcreate_f(MAX_POINTS,0);
      vsip_cvview_f *y = vsip_cvcreate_f(MAX_POINTS,0);
      vsip_vview_f  *xr = vsip_vrealview_f(x);
      vsip_vview_f  *xi = vsip_vimagview_f(x);
      vsip_cvview_f  *cramp = vsip_cvcreate_f(MAX_POINTS,0);
      vsip_vview_f  *ramp_r = vsip_vrealview_f(cramp);
      vsip_vview_f  *ramp_i = vsip_vimagview_f(cramp);
      vsip_cvview_f  *chk = vsip_cvcreate_f(MAX_POINTS,0);
      ...
      vsip_cvputlength_f(xc,1);vsip_cvputstride_f(xc,-1);
      vsip_vputlength_f(xnr,1);vsip_vputoffset_f(xnr,1);
      vsip_vputlength_f(xni,1);vsip_vputoffset_f(xni,1);
      vsip_cvputlength_f(xn,1);vsip_cvputoffset_f(xn,1);
      vsip_vramp_f(0,1,ramp_r);
      vsip_vfill_f(0,ramp_i);
      
      for(i=0; i<4; i++){
        printf("vsip_fft_f *fft = vsip_ccfftop_create_f(np,1.0,VSIP_FFT_FWD,0,0)\n");
        printf("np = %4d\n",np[i]);
        enp = np[i]; npm = np[i] / 2 - 1; t = M_PI / enp;
        vsip_cvputlength_f(x,np[i]);
        vsip_cvputlength_f(y,np[i]);
        vsip_cvputoffset_f(xc,np[i]-1);
        vsip_cvputlength_f(xc,npm);
        ...
        vsip_cvconj_f(xn,xc);
        vsip_cvputlength_f(x1,np[i]); vsip_cvputlength_f(y1,np[i]);
        vsip_cvcopy_f_f(x,x1); vsip_cvcopy_f_f(y,y1);
        { /* do fft */
          {
          vsip_fft_f *fft = vsip_ccfftop_create_f(np[i],1.0,VSIP_FFT_FWD,0,0);
          printf("x = ");VU_cvprintm_f("8.6",x1);
          vsip_ccfftop_f(fft,x1,y1);
          printf("vsip_ccfftop_f(fft,x,y)\n");
          printf("y = ");VU_cvprintm_f("8.6",y1);
        ...
        vsip_cvsub_f(y1,cramp,chk); vsip_cvmag_f(chk,chk_r);
        VU_limitIP_f(chk_r,.001);
