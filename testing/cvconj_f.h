/* $Id: cvconj.t,v 1.1 2000/04/02 20:29:58 judd Exp $ */
static void cvconj_f(){
   printf("********\nTEST cvconj_f\n");
   {
     ...
     vsip_cblock_f  *block = vsip_cblockbind_f(data_r,data_i,5,0);
     vsip_cblock_f  *ans_block = vsip_cblockbind_f(ans_r,ans_i,5,0);
     vsip_cvview_f  *a = vsip_cvbind_f(block,0,1,5);
     vsip_cvview_f  *ansv = vsip_cvbind_f(ans_block,0,1,5);
     vsip_cvview_f  *b = vsip_cvcreate_f(15,0);
     vsip_cvview_f  *chk = vsip_cvcreate_f(5,0);
     ...
     vsip_cvsub_f(b,ansv,chk); vsip_cvmag_f(chk,chk_r); VU_limitIP_f(chk_r,.0001);
     ...
     vsip_cvconj_f(a,a);
     vsip_cvsub_f(a,ansv,chk); vsip_cvmag_f(chk,chk_r); VU_limitIP_f(chk_r,.0001);
     if(vsip_vsumval_f(chk_r) > .5) 
         printf("error\n");
     else 
         printf("correct\n");
     ...
     printf("vsip_cvconj(a,b) with <a> stride 2, <b> stride 1\n");
     vsip_cvconj_f(b,a);
     vsip_cvsub_f(a,ansv,chk); vsip_cvmag_f(chk,chk_r); VU_limitIP_f(chk_r,.0001);
     if(vsip_vsumval_f(chk_r) > .5) 
         printf(" vconj_f in error for in place\n");
     else {
         printf("correct\n");
     }
